...
#
# /usr/local/etc/php/conf.d is symlinked to /etc/php-${BUILD_PHP_VERSION}/conf.d
# /etc/php-${BUILD_PHP_VERSION}/php-fpm.d
#
ARG PHP_VERSION=8.4
...
RUN echo "" \
    #&& ls /etc/php-${BUILD_PHP_VERSION}/conf.d/ \
    && rm -f /etc/php-${BUILD_PHP_VERSION}/conf.d/* \
...
# =========================
RUN echo "" \
    && echo "extension=exif.so" > /etc/php-${BUILD_PHP_VERSION}/conf.d/docker-php-ext-exif.ini \
    && echo "extension=gd.so" > /etc/php-${BUILD_PHP_VERSION}/conf.d/docker-php-ext-gd.ini \
    && echo "extension=gmp.so" > /etc/php-${BUILD_PHP_VERSION}/conf.d/docker-php-ext-gmp.ini \
    && echo "extension=ldap.so" > /etc/php-${BUILD_PHP_VERSION}/conf.d/docker-php-ext-ldap.ini \
    && echo "extension=pdo_mysql.so" > /etc/php-${BUILD_PHP_VERSION}/conf.d/docker-php-ext-pdo_mysql.ini \
    && echo "extension=imagick.so" > /etc/php-${BUILD_PHP_VERSION}/conf.d/docker-php-ext-imagick.ini \
    && echo "extension=redis.so" > /etc/php-${BUILD_PHP_VERSION}/conf.d/docker-php-ext-redis.ini \
    && echo ""
...
ENV PACKAGE_LIST_IMAGICK="libgomp imagemagick-libs lcms2 fftw-double-libs fontconfig libxext libltdl libexpat libraqm harfbuzz fribidi glib graphite2 libintl"
...
COPY --from=builder /etc/php-${BUILD_PHP_VERSION}/conf.d/ /etc/php-${BUILD_PHP_VERSION}/conf.d/
